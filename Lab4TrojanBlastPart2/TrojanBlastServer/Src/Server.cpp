...
{
	//make a random coin at 0,0 so we can test colliding with the player ship before a packet is set...
	GameObjectPtr go = GameObjectRegistry::sInstance->CreateGameObject( 'COIN' );
	go->SetLocation( XMVectorSet( 0.f, 0.f, 0.f, 0.f ) );
...
void Server::HandleNewClient( ClientProxyPtr inClientProxy )
{
	...
	ship->SetLocation( XMVectorSet(  1.f - static_cast< float >( playerId ), 0.f, 0.f, 0.f ) );
