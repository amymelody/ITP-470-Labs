...
void NetworkManager::ReadIncomingPacketsIntoQueue()
{
	...
	IncomingPacketBuffer packetBuffer;
	...
	{
		int readByteCount = mSocket->ReceiveFrom( packetBuffer, fromAddress );
		if( readByteCount == 0 )
		{
			//nothing to read
			break;
		...
		else if( readByteCount > 0 )
		{
			...
			if( MathUtils::GetRandomFloat() >= mDropPacketChance )
			{
				...
				mPacketQueue.emplace( simulatedReceivedTime, packetBuffer, fromAddress );
			}
			else
			{
				//dropped!
...
void NetworkManager::SendPacket( const OutgoingPacketBuffer& inPacketBuffer, const SocketAddress& inFromAddress )
{
	int sentByteCount = mSocket->SendTo( inPacketBuffer, inFromAddress );
	if( sentByteCount > 0 )
	{
...
NetworkManager::ReceivedPacket::ReceivedPacket( float inReceivedTime, IncomingPacketBuffer& ioIncomingPacketBuffer, const SocketAddress& inFromAddress ) :
	mReceivedTime( inReceivedTime ),
	mFromAddress( inFromAddress )
{
	using std::swap;
	//we don't want to copy the buffer- just steal it
	swap( ioIncomingPacketBuffer, mPacketBuffer );
}
