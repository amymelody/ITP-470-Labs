...
bool MemoryOutputStream::WriteBits( uint8_t inData, uint32_t inBitCount )
{
...
bool MemoryOutputStream::WriteBits( const void* inData, uint32_t inBitCount )
{
	if( mBitHead + inBitCount <= mBitCapacity )
	{
...
bool MemoryOutputStream::Write( const wstring& inData )
{
...
bool MemoryInputStream::ReadBits( uint8_t& outData, uint32_t inBitCount )
{
	if( mBitHead + inBitCount <= mBitCapacity )
	{
		uint32_t byteOffset = mBitHead >> 3;
...
bool MemoryInputStream::ReadBits( void* outData, uint32_t inBitCount )
{
	if( mBitHead + inBitCount <= mBitCapacity )
	{
...
bool MemoryInputStream::Read( wstring& outData )
{
	uint16_t length;
	uint32_t byteLength;
	if( Read( length ) )
